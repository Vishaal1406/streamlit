# pylint: disable= W0621,W0718,R1705,C0301
import os
# import json
import warnings
...
def get_weather_condition():
    ...
    try:
        # 5-second timeout for fast failures
...
def recommend_food():
    ...
    encoded_time = label_encoders["Time"].transform(
        [current_time])[0] if current_time in label_encoders["Time"].classes_ else -1
    encoded_month = label_encoders["Month"].transform(
        [current_month])[0] if current_month in label_encoders["Month"].classes_ else -1
    encoded_weather = label_encoders["Weather"].transform([current_weather])[
        0] if current_weather in label_encoders["Weather"].classes_ else -1

    # Convert input to NumPy array
    input_data = np.array(
        [[encoded_time, encoded_month, encoded_weather, weekend_special]])

    predictions = model.predict_proba(input_data)[0]
    sorted_indices = np.argsort(predictions)[::-1]
    ranked_food_items = label_encoders["Food Item"].inverse_transform(
        sorted_indices)
...
selected_language = st.selectbox(
    "Select Language:", list(
        LANGUAGES.keys()), key="language_select")
st.session_state.language = selected_language


@st.cache_data
def get_recommendations(cache_key):
    _ = cache_key
    return recommend_food()

if "recommendation_key" not in st.session_state:
    st.session_state.recommendation_key = 0  # Initialize

recommended_items = get_recommendations(st.session_state.recommendation_key)
...
if st.session_state.page == "Menu":
    ...
    for i, item in enumerate(recommended_items):
        ...
        with cols[i % 3]:
            ...
            if st.button(
                    translate_text(f"Add {item} to Cart"),
                    key=f"recommend_{item}"):
                add_to_cart(item, 0, img_path)
    if st.button(translate_text("Refresh Recommendations")):
        st.session_state.recommendation_key += 1  # type: ignore
        st.rerun()
    categories = {
        "Breakfast": [
            ("Idli", 30, "idli.jpg"),
    ...
    for category, items in categories.items():
        ...
        for i, (item, price, img_name) in enumerate(items):
            ...
            with col:
                img_path = os.path.join(
                    script_dir, "images", f"{item.lower().replace(' ', '_')}.jpg")
    ...
    if st.session_state.cart:
        ...
        for item, details in st.session_state.cart.items():
            ...
            with col4:
                #details = st.session_state.cart[item]  # Store reference once
                if details['quantity'] > 1:
                    details['quantity'] -= 1
                else:
                    items_to_remove.append(item)  # No redundant dictionary lookup
