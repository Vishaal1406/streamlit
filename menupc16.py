...
if st.session_state.page == "Menu":
    ...
    st.write(f"**{translate_text("Service Cost")}: {service_charge}**")
    ...
    tax=sgst+cgst+service_charge
    final_total = subtotal + tax
    st.markdown(f"### 🛒 Order Total: **₹{final_total}**")

    payment_method = st.selectbox("Select Payment Method", ["Credit Card", "Debit Card", "UPI", "NEFT", "QR Code"])
    if payment_method in ["Credit Card", "Debit Card"]:
        card_type = st.selectbox("Select Card Type", ["Visa", "MasterCard", "RuPay", "Amex"])
    ...
    # NEFT Input (Sender's Bank Details)
    elif payment_method == "NEFT":
        st.subheader("Sender's Bank Details")
        sender_name = st.text_input("Sender’s Full Name")
        sender_bank = st.text_input("Sender’s Bank Name")
        sender_account = st.number_input("Sender’s Account Number", step=1, format="%d")
        sender_ifsc = st.text_input("Sender’s IFSC Code")
        sender_branch = st.text_input("Sender’s Branch Name")

        st.subheader("Receiver’s Bank Details")
        st.text("Name: Indian Restraunt")
        st.text("Receiver’s Account Number: 112233476566")
        st.text("Bank Name: State Bank of India")
        st.text("Branch Name: Vadapalani Branch")
        st.text("Receiver’s IFSC Code: SBIN0007587")


    elif payment_method == "QR Code":
        col1, col2, col3 = st.columns([2, 2, 2])  
        with col2:
            st.image("qr_code.png", caption="Scan the QR Code to Pay")
    ...
    if st.session_state.bill_pdf:
        ...
        <h3 style="margin: 5px 0; font-size: 18px; font-weight: bold;">INDIAN RESTAURANT</h3>
